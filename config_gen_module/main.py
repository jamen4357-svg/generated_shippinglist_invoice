...
"""
    ...
    if args.output:
        # If user provides a path, use it directly. Otherwise, use the default dir.
        final_output_path = Path(args.output)
        if len(final_output_path.parts) == 1: # It's just a filename
            final_output_path = output_dir / final_output_path
    else:
    ...
    if args.generate_xlsx and XLSX_GENERATOR_AVAILABLE:
        ...
        try:
            ...
            # Determine the XLSX output path
            if args.xlsx_output:
                xlsx_output_path = Path(args.xlsx_output)
                if len(xlsx_output_path.parts) == 1: # It's just a filename
                    xlsx_output_path = output_dir / xlsx_output_path
            else:
