...
class PositionUpdater:
    ...
    def update_footer_configurations(self, template: Dict[str, Any], quantity_data: QuantityAnalysisData) -> None:
        ...
        for sheet_name, sheet_config in data_mapping.items():
            ...
            # Handle merged cells in footer to create merge_rules
            footer_merge_rules = []
            if hasattr(sheet_data, 'footer_info') and sheet_data.footer_info and hasattr(sheet_data.footer_info, 'merged_cells') and sheet_data.footer_info.merged_cells:
                merged_cells = sheet_data.footer_info.merged_cells
                
                print(f"🔗 [FOOTER_CONFIG] Processing {len(merged_cells)} merged cells in footer for {sheet_name}")
                
                # Track merge rules by start column to avoid duplicates
                merge_rules_by_column = {}
                
                for excel_column, span_info in merged_cells.items():
                    # Extract span information
                    if isinstance(span_info, dict):
                        colspan = span_info.get('colspan', 1)
                        rowspan = span_info.get('rowspan', 1)
                        start_row = span_info.get('start_row', 0)
                        end_row = span_info.get('end_row', 0)
                    else:
                        # Fallback for old format (just colspan)
                        colspan = span_info
                        rowspan = 1
                        start_row = 0
                        end_row = 0
                    
                    # Use raw index format (0-based) as requested
                    raw_index = excel_column - 1  # Convert Excel 1-based to 0-based index
                    
                    # Check if we already have a merge rule for this start column
                    if raw_index in merge_rules_by_column:
                        existing_rule = merge_rules_by_column[raw_index]
                        # Keep the rule with the larger colspan (or combine if needed)
                        if colspan > existing_rule['colspan']:
                            print(f"🔄 [FOOTER_CONFIG] Updating merge rule for index {raw_index}: {existing_rule['colspan']} → {colspan} columns")
                            merge_rules_by_column[raw_index] = {
                                "start_column_id": raw_index,
                                "colspan": colspan
                            }
                            if rowspan > 1:
                                merge_rules_by_column[raw_index]["rowspan"] = rowspan
                        else:
                            print(f"⏭️ [FOOTER_CONFIG] Skipping duplicate merge rule for index {raw_index} (smaller span: {colspan} vs {existing_rule['colspan']})")
                    else:
                        # New merge rule
                        merge_rule = {
                            "start_column_id": raw_index,
                        ...
                        merge_rules_by_column[raw_index] = merge_rule
                        print(f"📏 [FOOTER_CONFIG] Added merge rule: index {raw_index} spans {colspan} columns, {rowspan} rows")
                
                # Convert to list and sort by start column for consistent output
                footer_merge_rules = [merge_rules_by_column[col] for col in sorted(merge_rules_by_column.keys())]
                
                if footer_merge_rules:
                    footer_updates['merge_rules'] = footer_merge_rules
                    print(f"✅ [FOOTER_CONFIG] Will add {len(footer_merge_rules)} unique merge rules to {sheet_name} footer")
