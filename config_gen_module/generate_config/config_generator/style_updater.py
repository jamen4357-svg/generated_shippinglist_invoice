...
class StyleUpdater:
    ...
    def update_number_formats(self, template: Dict[str, Any], quantity_data: QuantityAnalysisData) -> Dict[str, Any]:
        ...
        try:
            ...
            for sheet_data in quantity_data.sheets:
                ...
                if number_formats:
                    sheet_config['styling']['number_formats'] = number_formats
                    print(f"✅ [NUMBER_FORMATS] Updated {mapped_sheet_name}: {number_formats}")
                else:
    ...
    def _extract_number_formats_from_sheet(self, sheet_data: SheetData, sheet_name: str) -> Dict[str, str]:
        """
        Extract number formats from actual Excel file by analyzing cell patterns.
        
        Args:
            sheet_data: Sheet data containing basic information
        ...
        # Try to extract from actual Excel file if available
        try:
            # Get the Excel file path from the analysis data
        ...
        # Fallback: Don't use hardcoded defaults, return empty dict
        print(f"[NUMBER_FORMATS] No number formats extracted for {sheet_name} - using existing column_id_styles")
        return {}  # Let existing column_id_styles handle formatting
